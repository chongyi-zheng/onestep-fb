...
class GCIntentionEncoder(nn.Module):
    ...
    state_encoder: nn.Module = None
    goal_encoder: nn.Module = None
    intention_encoder: nn.Module = None
    concat_encoder: nn.Module = None
    ...
    def __call__(self, observations, goals=None, intentions=None, goal_encoded=False, intention_encoded=False):
        ...
        if intentions is not None:
            if intention_encoded:
                ...
                if self.intention_encoded is not None:
                    reps.append(self.intention_encoder(intentions))
                if self.concat_encoder is not None:
                    if len(concat_encoder_inputs) == 0:
                        concat_encoder_inputs.append([observations, intentions])
                    else:
