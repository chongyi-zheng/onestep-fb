...
class LaplacianAgent(flax.struct.PyTreeNode):
    """Laplician representation learning agent.
    ...
    def repr_loss(self, batch, grad_params):
        ...
        next_phis = self.network.select('obs_repr')(next_observations, params=grad_params)
        phi_loss = jnp.mean((phis - next_phis) ** 2)
        
        # Orthonormalization regularization.
        ortho_diag_loss = -(jnp.sum(phis * phis, axis=-1) ** 2 + jnp.sum(next_phis * next_phis, axis=-1) ** 2)
        ortho_diag_loss = jnp.mean(ortho_diag_loss)
        ortho_off_diag_loss = jnp.mean(jnp.sum(phis * next_phis, axis=-1) ** 2)
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        # Train the Laplacian representations.
        repr_loss, repr_info = self.repr_loss(batch, grad_params)
        for k, v in repr_info.items():
            info[f'repr/{k}'] = v

        # Train the critic using intrinsic rewards defined by the Laplacian representations.
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='laplacian',  # Agent name.
